...
###############################################################################
{
    ...
    while ( ($RuleKey,$ValDef) = each(%$RulePointer) )
    {
    	...
    	defined $G_CLIParams{D} and WrLog("Debug - Rul Key = \"$RuleKey\" , Value = \"$ValDef\" Operation \"$OpCh\"");
	...
	exists $G_CLIParams{D} and WrLog("Debug - Start Number manipulation ...");
	foreach my $Op ( (">","<","!") ) 
	{
		while ( my ($PVal,$PCal) = each(%{$ListDet{$Op}}) ) 
		{
			...
			exists $G_CLIParams{D} and WrLog("Debug -  Operation Found \"$Op\" ($FlagMatch .. $PVal > $ActValue ) ....");
			$FlagMatch or next;
			...
			foreach my $Val (split(',',$PCal) )
    		{
				...
				if ( $Val =~ /^[\d\s\*\-\+\%\^\(\)\)\/\\\.]+$/ ) 
				{
					my $CalcExp= $Val =~ /^\s*[\+\-\*\/\%\^]/ ? "$ActValue $Val" : $Val ;
...
WrLog(@DebugMessages);
my (%Configuration,%CLIOctParams,%TmpOctopusParams,%TmpMacros);
...
{
	%Configuration=LoadIniFile($ConfFile);
	%TmpOctopusParams=ReadSections(\%Configuration,"Values");
	%CLIOctParams=(%CLIOctParams,%TmpOctopusParams);
	%TmpOctopusParams=ReadSections(\%Configuration,"Macros");
	%TmpMacros=(%TmpMacros,%TmpOctopusParams);
	WrLog("Info - Add Values Section of Configuration file $ConfFile");
}
%CLIOctParams=(%CLIOctParams,%G_CLIParams);
...
$G_ErrorCounter += SetSection(\%Configuration,"Macros",%TmpMacros);
%TmpOctopusParams=ReadSections(\%Configuration,"Values");
