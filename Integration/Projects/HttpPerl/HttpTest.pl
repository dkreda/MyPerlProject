...
WrLog(@DebugMessages);
WrLog("Info - Retreives $G_CLIParams{Url} ...");
my $PgStr=get($G_CLIParams{Url});
my @Content=split("\n",$PgStr);
chomp @Content;
WrLog("","-",@Content,"--");
###   Retrieve Only Scripts ... #
my $StartScript=-1;
for ( my $i=0 ; $i < @Content ; $i++ )
{
	$Content[$i] =~ /<script/i and $StartScript = $i;
	$Content[$i] =~ /<\/script/i or next;
	WrLog("Script:",@Content[$StartScript .. $i],"\t>===<");
}
WrLog("--==--","===========================================================");
