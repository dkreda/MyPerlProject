...
my @CmdList=( "mkdir -p $G_SWIMConf{SysDir}/$Conf{NewName}" ,
			  ...
			  "cp -f $G_SWIMConf{Kits}/AddService/$Conf{SysType}/*-UAF.xml $G_SWIMConf{SysDir}/$Conf{NewName}" ,
			  "cp -f $G_SWIMConf{Kits}/AddService/$Conf{SysType}/$Conf{SysType}-SAF.xml $G_SWIMConf{SysDir}/$Conf{NewName}/$Conf{NewName}-SAF.xml" );
...
my %ServiceMap = (  "MMG"		=> {"MZ Balancers IP List" => 
					...
					VI			=> {} ,
					"NDU-WHC"	=> {"Site Name"	=> $Conf{"PVVM"} , "NDU System Name" => $Conf{SysName} } );
...
foreach my $FileName ( @FilesList )
{
	...
	while ( my ($Name,$PVal) = each( %{$ServiceMap{$Conf{SysType}}} ) )
	{
		...
		if ( $PVal =~ /\{(.+?)::(.+)\}/ )
		{
			my $XmlFile=$1;
			my $XPath=$2;
			-e $XmlFile or WrLog("Error - Fail to update Parameter \"$Name\" Value. The File $XmlFile not exists"),$G_ErrorCounter++,next;
			my $TmpXml=XML::LibXML->new()->parse_file($XmlFile);
			$PVal=$TmpXml->findvalue($XPath);
		}
