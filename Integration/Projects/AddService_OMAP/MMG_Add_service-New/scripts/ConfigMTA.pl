...
{
    $0 =~ s/^.+[\\\/]// ;
    print "$0 [-LogFile FileName] [-MTAPass password] [-Answer FileName] [-UnitType UnitType]\n";
    print "Description:\n";
    print "-----------------------------------------------------------\n";
...
sub chk
{
  my ($output, $res, $mode) = @_;
  WrLog("$output status $res");
  if(($mode == $DO_ABORT) && ($res != 0))
  {
    close(LOGFILE);
    exit 1;
  }
...
###############################################################################
{
    ...
    unless ( IsMTAup("StartMTA") )
    {
    	WrLog("Error\tFail To Read List - MTA is down.");
    	$G_ErrorCounter++;
    	return "";
    }
...
###############################################################################
{
   ...
   my @MgrCmds=("REMOTE DOMAIN ADD $Domain",
   		"REMOTE ROUTE $Domain2 ADD $Host PORT=$Port"); 
   if ( $Port <= 0 )
   {
   	$MgrCmds[1] =~ s/=.$/=50025/ ;
   }
...
###############################################################################
{
	...
	unless ( $Input =~ /(\S+)\s+"(.+)"?/ )
   {
   	WrLog("Error - RelayConfig format error \"$Input\"");
   	return  1;
   }
   my $Domain=$1;
   my $BODelay=$2;
   chop ($BODelay);
   my @MgrCmds=("REMOTE CONFIG $Domain SET BackoffDelay $BODelay"); 
   return  @MgrCmds;
}
...
###############################################################################
{
    my $IsVersionFile="/var/tmp/ISVersion.txt";
	my $Cmd="version > $IsVersionFile";
	my ($ISVersion,$ISVersionNum)=0;
	RunCmds("$Cmd") and EndProg(1,"Fatal - IS4 Version cannot be determined.");
    my @LogLines=ReadFile($IsVersionFile);
    foreach (@LogLines)
    {
    	$_ =~ /^InSight Release/ or next;
		$_ =~ /(\d\S+)(-\d)/ and $ISVersion=$1;
		WrLog("Debug - InSight4 Version is $ISVersion");
	}
	foreach my $Octec ( split(/\./,$ISVersion))
    {
    	$ISVersionNum= $ISVersionNum*10+$Octec;
    }
	return $ISVersionNum;
}
...
sub AddMTAParam #$Parametr , $Value
{
	...
	my $Cmd=();
	my $Err=0;
	if (($Parameter eq "PendingQueueChunkSize") or ($Parameter eq "SmoothDeliveryBursts"))
	{
	    $Cmd="GET $Parameter";
		$Err=RunMgr($Cmd);
	    if ($Err)
...
sub verify_mips_resource
{

  if (! -e "/usr/cluster/bin/scstat") {
      WrLog("INFO:      /usr/cluster/bin/scstat was not found. Assuming single-node configuration. Continue.");
      return;
  }  
  my $online_node = "";
  ...
  my $status = $?;
  chomp $hostname;
  chk("COMMAND 'hostname'" , $status, $DO_ABORT);
  WrLog("INFO    Running on $hostname");
  
  my $command = "scstat | grep \"Group: mips\"";
  my @scstat = `$command`;
  $status = $?;
  WrLog("INFO    output of '$command':\n@scstat");
  chk("COMMAND $command" , $status, $DO_ABORT);
  foreach my $line (@scstat) 
  {
    chomp $line;
    if ($line=~/\s+Group: mips\s+(\S+)\s+Online/) 
    {
      $online_node=$1;
   	  WrLog("INFO    MIPS is running on $online_node");
      last;
    }
  }
  if (! defined $online_node) 
  {
    chk("ERROR   Did not find were MIPS is running, Check MIPS status in Cluster" , 1, $DO_ABORT);
  }

  if ($hostname ne $online_node)
  {
    WrLog("INFO:    MIPS is not running on this node. Nothing to do");
    EndProg(0, "MIPS is not running locally. Nothing to do.");
  }
  if ($hostname eq $online_node)
  {
    WrLog("INFO:    MIPS is running on this node. Continue.");
  }
}
...
if (($G_CLIParams{UnitType} eq "VM-ASU") and ($G_CLIParams{Answer} =~ /Resilience/i))
{
    my $ISVersion = VerifyInsightVersion();
	if ($ISVersion>4432)
	{
		EndProg(1,"Error - InSight4 Version is Higher than required - Due to this fact no MTA Configuration will be done"); 
	}
}
verify_mips_resource();
...
if ( $G_ErrorCounter ) { EndProg(1,"Fatal - Fail to Clear / Read from MTA Configuration"); }
