...
WrLog(@DebugMessages);
my @Content=ReadFile("/etc/hosts");
my (%IpList,@NewContent);
foreach my $Line (@Content)
{
	$Line =~ /^\s*((\d+\.){3}\d+)/ or push(@NewContent,$Line),next;
	my $IpKey ;
	my @Domains=split(/\s+/,$Line);
	$IpKey=shift(@Domains) while ( not $IpKey );
	for my $Iter (@Domains)
	{
		if ( exists $IpList{$IpKey} )
		{
			grep(/^$Iter$/i,@{$IpList{$IpKey}->{Domains}}) and next ;
			$Iter =~ /\S\.\S/ ? unshift(@{$IpList{$IpKey}->{Domains}},$Iter) : push(@{$IpList{$IpKey}->{Domains}},$Iter) ;
		} else
		{
			$IpList{$IpKey}->{Domains}=[$Iter];
			push(@NewContent,$Line);
			$IpList{$IpKey}->{LineNumber}=$#NewContent;
		}
	}
}

while ( my ($IPAddr,$Rec) = each(%IpList) )
{
	$NewContent[$Rec->{LineNumber}]=sprintf("%-15s\t%s",$IPAddr,join(' ',@{$Rec->{Domains}}));
}
$G_ErrorCounter=UpdateFile("/etc/hosts",@NewContent);
