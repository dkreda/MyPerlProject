...
-e $G_CLIParams{File} or EndProg(1,"Error - coold not find file \"$G_CLIParams{File}\"");

my $XmlObj=XML::LibXML->new()->parse_file($G_CLIParams{File});
my $Counter=0;
foreach my $Xpath ( split(',',$G_CLIParams{Xpath}) ) 
{
	my @DeleteNodeList=$XmlObj->findnodes($Xpath);
	foreach my $NodeIns ( @DeleteNodeList) 
	{
		my $DebNode=$NodeIns->parentNode()->removeChild($NodeIns);
		WrLog(sprintf("Info  - The Node \"%s\" removed ...",$DebNode->nodeName()));
		$Counter++;
	}
}
WrLog("Info- Total $Counter Nodes have been deleted");
if ( BackupFile($G_CLIParams{File}) ) 
{
	$G_ErrorCounter += $XmlObj->toFile($G_CLIParams{File},1) ? 0 : 1;
} else 
{
	$G_ErrorCounter += 10;
}
