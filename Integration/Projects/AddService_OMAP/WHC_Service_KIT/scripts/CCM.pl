...
my $G_ErrorCounter=0;
my %G_CLIParams=(  LogFile  => "-,/tmp/WHC_CCMSupport.log" );
my @G_FileHandle=();
...
{
	my $SystemName=shift;
    my $SwimUnitGroupFileName=$SystemName . "CCMUnitGroup.xml";
...
###############################################################################
{
    ...
    my ($UnitGroupHostName , $SwimUnitGroupHostName , $UnitHostname , $UnitIP , $UnitTypeInst);
	...
	my @UnitList=$RootObj->findnodes("//Logical/Application [\@ApplicationLabel=\'CCM Agent\']/UnitInstance");
    foreach my $UnitInst (@UnitList)
    {
	    $UnitTypeInst=$UnitInst->getAttribute("ParentGroup");
		$UnitTypeInst=uc($UnitTypeInst);
		($UnitTypeInst eq "NDU-WHC_UNIT") or next;
	...
	if (!keys %UnitGroupIpList)
	{
		EndProg (1,"ERROR - There are no WHC Units that appears in UnitGroup in CCM Agent Section");
	}
	WrLog ("WHC units as appears in UnitGroup in CCM Agent Section:\n");
	...
	if (!keys %SwimUnitGroupIpList)
	{
		EndProg (1,"ERROR - There are no WHC Units that appears in SWIM UnitGroup ");
	}
        ...
        if (exists $UnitGroupIpList{$UnitIPIter})
        {
            ...
            else
            {
                WrLog ("WARN - Unit $UnitIPIter appears in the both UnitGroups ,","WARN - But its Hostname is different between SWIM UnitGroup = $SwimUnitGroupHostName , and SCDB UnitGroup = $UnitGroupHostName",
                       "WARN - The script will continue and CCM will use the Hostname as appears in SCDB UnitGroup = $UnitGroupHostName");
                next;
            }
        }
        else
    ...
    WrLog ("All Units appears both in SWIM UnitGroup and SCDB UnitGroup , continue to CCM procedures...");
}
...
my $CCMApplVersion=`rpm -qa --last | grep -i ccmappl | head -1`;
$CCMApplVersion =~ s/^.+?-(\d[\d\.\-_]+).+$/.$1/;  
$CCMApplVersion =~ s/-/_/;
chomp ($CCMApplVersion);
WrLog ("CCM Application Version is $CCMApplVersion , and the folder that SystemList should be placed is /usr/cti/conf/ccm-app$CCMApplVersion/scdb/SystemList.xml");
### Verify System Name via SystemList file  ###
WrLog ("Verifying that SystemList file is located in CCM Configuration folder");
my $SystemListFile="/usr/cti/conf/ccm-app$CCMApplVersion/scdb/SystemList.xml";
if (! -f $SystemListFile)
{
	EndProg (1,"SystemList.xml file does not exist in CCM Folder , please fix it using the correct CLI before Re-running the script");
}
WrLog ("SystemList.xml file is located in CCM folder");
...
WrLog ("The System Name that you are working with appears in SystemList.xml file");
### Verify UnitGroup file exists in SystemName folder ###
WrLog ("Verifying that UnitGroup file is located in CCM System Name=$SystemName folder");
my $UnitGroupFile="/usr/cti/conf/ccm-app$CCMApplVersion/scdb/$SystemName/UnitGroup.xml";
unless ( -f  $UnitGroupFile )
{
	EndProg(1,"ERROR - UnitGroup file does not exists in the following location \"$UnitGroupFile\"");
}
...
WrLog ("Verifying that CCM is online");
if ( RunCmds("ps -ef | grep -i ccm-appl | grep -v grep") )
{
	EndProg(1,"Fatal - CCM is down");
}

### Updating CCM Topology ##### 
WrLog ("Running CCM Rebuild Topology");
$G_ErrorCounter += RunCmds("su - ccm_user -c \"ccm-cli -C updateTopology -s $SystemName -o BUILD\"");
