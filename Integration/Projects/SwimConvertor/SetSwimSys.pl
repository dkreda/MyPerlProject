...
sub JoinCafs #\@CafFilesNames,@UAfList
{
    ...
    foreach my $ListPointer (@MatchList)
    {
		...
		my @Cmds=(sprintf("perl -MExtUtils::Command -e rm_f %s",join(" ",@DelList)));
		$^O =~ /Linux/i and unshift(@Cmds,sprintf("chown --reference=$DelList[0] %s",join(" ",@MatchFiles)));
		$Err or $Err += RunCmds(@Cmds); 
    }
	return $Err;
}
...
sub InsertAlias #$XmlObj,$AliasName,$ParamName,$CompName,$ParentTag,$RestNode,$ValNode
{
	...
	unless ( @TmpList )
	{
			...
			###  Check if this parameter already configured as parameter at file ....
			$ValNode and $AliasNode->appendChild($ValNode);
			$AliasNode->appendChild($RestNode);
	}
	@TmpList=$AliasNode->findnodes("./Parameter[\@Name=\"$ParamName\" and \@ComponentName=\"$CompName\"]") ;
	if ( @TmpList > 0 )
	{
		exists $G_CLIParams{D} and WrLog("Debug - Parameter $ParamName already exists at Alias $AliasName skip update");
		return ;
	}
	@TmpList=$XmlObj->findnodes("//Parameters/Parameter[\@Name=\"$ParamName\" and \@ComponentName=\"$CompName\"]") ;
	if ( @TmpList )
	{
			my @StamList=$TmpList[0]->findnodes(".//Value");
			@StamList and $AliasNode->insertBefore($StamList[0], $AliasNode->firstChild) ;
			$TmpList[0]->parentNode->removeChild($TmpList[0]);
	}
...
sub SetAlias #\@CafFilesNames,\%Conf,$SafXmlObj,@UAfList
{
	...
	foreach my $CafFile (@$CafFiles)
	{
		...
		my @TmpUafList=();
		exists $G_CLIParams{D} and WrLog("Debug - Analyze $CompName ....");
		foreach my $Tmp (@UAfRecList)
		{
			...
			exists $G_CLIParams{D} and WrLog("Debug - Componnet $CompName is relevant for $Tmp->{FileName}");
		}
...
my $SafXmlObj = -e $SafFileName ? XML::LibXML->new()->parse_file($SafFileName) : XML::LibXML->new()->parse_string("<SWIM/>");
...
if ( @UAFs )
{
	...
	### UpDate at SAF all The UnitsType of the UAFs - means add all UAFS to SAF ... ###
	##     TODO -  Add Multiple UAF Files for single UnitType .... //UnitType/UAF/@Name ...  ##
	...
	exists $G_CLIParams{D} and WrLog("Debug - Search for Alias ...");
	...
	unless ( $G_ErrorCounter )
	{
		...
		foreach my $UAFRec ( @UAFs )
		{
			...
			foreach my $AliasObj ($UAFRec->{Xml}->findnodes("//Alias"))
			{
				...
				my @TmpList=$UAFRec->{Xml}->findnodes(sprintf("//Parameter[\@Name=\"%s\" and \@ComponentName=\"%s\"]",
										$ListNum[0]->getAttribute("Name"),$ListNum[0]->getAttribute("ComponentName")));
				if ( @TmpList )
				{	### Change alias to parameter  ###
					$ListNum[0]->setAttribute("Display","true");
					my @ValNodes=$AliasObj->findnodes(".//Value");
					@ValNodes and $ListNum[0]->appendChild($ValNodes[0]->cloneNode(1));
					$AliasObj->parentNode()->replaceChild($ListNum[0], $AliasObj);
					WrLog(sprintf("Info - Convert alias %s to parameter. less than 2 parameters", $AliasObj->getAttribute("DisplayName")));
				} else
				{	###  Delete the Alias
					$AliasObj->parentNode()->removeChild($AliasObj);
					WrLog(sprintf("Info - remove alias %s. No Parameters found", $AliasObj->getAttribute("DisplayName")));
				}
			}
