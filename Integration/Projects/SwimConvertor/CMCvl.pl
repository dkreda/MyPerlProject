...
####  Search CVL (PSG Files)  ###
my @FolderList=SearchFiles($G_CLIParams{Kits},"PSG\\d+.txt");
WrLog("Info - Find the following PSG Files:",@FolderList);
my %Map;
###  Start Parse all Cvls ....
foreach my $CVLFile ( @FolderList )
{
	...
	foreach my $Line ( @CVLContent )
	{
		...
		my $Dir=join('.+',($Rec[0],$Rec[1],$Rec[2]));
		my @TmpArray=@Rec[0,1,2,6];
		@FolderList=SearchFiles($G_CLIParams{Kits},"$Dir\$");
		foreach $Dir ( @FolderList )
		{
			WrLog("Debug - Seraching Caf File at $Dir");
			foreach my $CafFileName (SearchFiles($Dir,"CAF.xml\$"))
			{
				WrLog("Debug - Parsiing File $CafFileName ...");
				my $TmpXmlObj=XML::LibXML->new()->parse_file($CafFileName);
				my $CompName=$TmpXmlObj->findvalue("//Component/\@Name");
				$CVLCompList{$CompName}=$Rec[0];
			}
		}
	}
}
...
keys(%CVLCompList) > 1 or EndProg(1,"Error - No Components Found at CVL ...");
my @CvlContent = ("Compinent Name,CI Name");
while ( my ($CompName,$CIName) = each(%CVLCompList) )
{
	push(@CvlContent,"$CompName,$CIName");
}
unless ( $G_ErrorCounter )
{
	WrLog("Debug - start Build CVL ....");
	WriteFile($G_CLIParams{Target},@CvlContent);
}
