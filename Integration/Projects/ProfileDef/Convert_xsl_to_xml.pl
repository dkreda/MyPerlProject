...
{
	if ( @ARGV != 2 ) {
           $logger->error("ERROR - [USAGE]: $0 [xls file] [current ProfileDefinition_InSight.xml]") ;
           exit 1;
    } else {
...
sub ProfDefParser
{
	...
	open(FH, ">", $NewXML) or do
	{
		doAbort("Failed to create $NewXML: $!",3);
	};
...
sub readXLS
{
	...
	$source_excel = new Spreadsheet::ParseExcel;
	$parse_excel = $source_excel->Parse($ExcelFile)
	 or die print "Could not open source Excel file $ExcelFile: $!";
	
		
	#Read sheets	
	foreach my $worksheet ('IS4 Subscriber', 'Subscriber-related tables') 
	{
		 ...
		 if ($worksheet eq 'IS4 Subscriber') {
			 $attr_col = 1;
			 $type_col = 4;
			 $len_col = 5;
			 $sysdef_col = 12;
