...
sub Banner ### String
{
	...
	my $FontHigh=8;
	my @Result;
	for ( my $WordIndx=0 ; $WordIndx < length($Line) ; $WordIndx++ )
	{
		my $Indx=substr($Line,$WordIndx,1);
		if ( exists $G_Font{$Indx} )
		{
			my $FIndx=1;
			for ( my $i=1; $i<= $FontHigh ; $i++ )
			{
				my $Prn=exists $G_Font{$Indx}->[$FIndx] ?  $G_Font{$Indx}->[$FIndx] : "$i" ;
				$Prn =~ s/(\d+)// or WrLog("Error - $Indx do not have Starting Number at Line $i"),next ;
				my $LinIndx=$1;
				my $Str =  sprintf("%*s",-1 * $G_Font{$Indx}->[0], $i eq $LinIndx ? $Prn : ""  );
				$LinIndx eq $i and $FIndx++;
				$Result[$i-1] .= $Str;
			}
		} else
		{
			WrLog("error - dont have map for OT $Indx");
		}
	}
	return @Result;
}
...
WrLog(@DebugMessages);
my @TmpConf=ReadFile($G_CLIParams{Banner});
my $OT;
foreach my $Line ( @TmpConf )
{
	if ( $Line =~ /\[(\S+?)\]/ )
	{
		$OT=$1;
		exists $G_Font{$OT} or $G_Font{$OT}=[0];
		next;
	}
	if ( $Line =~ /\S/ )
	{
		push(@{$G_Font{$OT}},$Line);
		$G_Font{$OT}->[0] < length($Line) and $G_Font{$OT}->[0]=length($Line);
	} else
	{
		$OT=lc($OT);
		exists $G_Font{$OT} or $G_Font{$OT}=[0];
...
foreach my $TxtLine (split(/,/,$G_CLIParams{Text}))
{
	WrLog(Banner($TxtLine));
}
