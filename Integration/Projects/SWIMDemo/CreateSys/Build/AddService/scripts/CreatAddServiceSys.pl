...
my %ServiceMap = (  "MMG"		=> {"MZ Balancers IP List" => 
					...
					"NDU-WHC"	=> {"Site Name"	=> $Conf{"PVVM"} , 
									"SWIM UnitGroup Location" => $Conf{SysName} ,
...
foreach my $FileName ( @FilesList )
{
	...
	while ( my ($Name,$PVal) = each( %{$ServiceMap{$Conf{SysType}}} ) )
	{
		...
		foreach my $Xpath (@XpathList)
		{
			...
			@ObjList or next;
			@ValObj=$ObjList[0]->findnodes(".//Value");
			$ObjNode=$ObjList[0];
			last;
		}
		$ObjNode or next;
		...
		if ( @ValObj )
		{
			$ObjNode->replaceChild($NewVal,$ValObj[0]);
		} else
		{
			$ObjNode->insertBefore($NewVal,$ObjNode->firstChild());
		}
		$FlagCounter++;
	}
