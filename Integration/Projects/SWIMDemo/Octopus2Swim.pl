...
my %G_CLIParams=(  LogFile  => "-" ,
		   DefaultDir => "/tmp" ,
		   SearchLevel	=> 2 ) ;
my %G_ReservedParams=(	Local_IP		=> "{LocalIP}" ,
						Unit_Type		=> "{UnitType}" ,
						UnitType		=> "{UnitType}" ,
						HostName		=> "{UnitName}" ,
						ComponentVersion => "{ComponentVersion}" ,
						"3DES_KEY"		=> "010215050E15000B7A711F47D95647FF45FDB7D88CF8090F62736C33FFE6D4515E8B5DE4AAD13EB16E915116A7DBB71872854DD209B6723DA3676AC578A45EBFD0F627026EC0DA5CD857D7BB697E99C4" 
						) ;
my @G_FileHandle=();
...
sub setUnitCAF  ### \%MapRec,$UnitType,$CompName,$CompVer,$FilePath
{
	...
	if ( exists $MapRec->{$UnitType}->{VFlag} )   ## or return 0;
	{
		my $ProductName=$MapRec->{$UnitType}->{VFlag};
		## WrLog("Debug -   Setting Virtual component \"$UnitType\" as product ($ProductName)!!!! :");
		while ( my ($UnitName,$UnitRec) = each(%$MapRec) )
		{
			exists $UnitRec->{VFlag} and next;
			exists $UnitRec->{$ProductName} or next;
			## WrLog("Debug - Component $CompName _ $UnitType should be add to $UnitName ...");
			exists $UnitRec->{UAFXml} or next;
			my $UAfCompNode=ValidatePath($UnitRec->{UAFXml},"//SWIM/UnitType/Install");
			$UAfCompNode=$UAfCompNode->appendChild(XML::LibXML::Element->new("Component"));
			$UAfCompNode->setAttribute("Name","$CompName"."_$UnitType");
			$UAfCompNode=$UAfCompNode->appendChild(XML::LibXML::Element->new("OnlyOn"));
			$UAfCompNode->appendTextChild("UnitName","\{$UnitType\}");
			## WrLog("Debug - Component $CompName _ $UnitType have been add to $UnitName with OnlyOn condition...");
		}
	} else
	{
...
###############################################################################
{
	...
	if ( $GenTable ) 
	{
		exists $G_CLIParams{D} and WrLog("Debug - Parsing Version Column");
...
sub AddPath
{
	...
	$Result->setAttribute("ExecutionOrder",$ExecOrd);
	$Row->[7] and  $Result->setAttribute("Timeout",$Row->[7]);
	$Result->appendTextChild("Command","PATH=\$PATH:$Row->[3] \&\& export PATH");
	return $Result;
}
...
sub unZip
{
	my $Row=shift;
	my $ExecOrd=shift;
	unless ( $Row->[4] and $Row->[3] )
	{
		WrLog("Error - Unzip Command have missing Target ( or Source ): unzip , $Row->[3] ,$Row->[4]");
		return ;
	}
	$Row->[3] =~ /([^\/\\]+)$/ or WrLog("Error - Wrong File Format at Unzip Command: $Row->[3]") , return ;
	my $ZipFile= $1;
    my $Target= $Row->[4] ;
	my $Result=XML::LibXML::Element->new("Execute");
	## WrLog("Debug - Set Attribute  ...");
	$Result->setAttribute("ExecutionOrder",$ExecOrd);
	$Row->[7] and  $Result->setAttribute("Timeout",$Row->[7]);
	$Result->appendTextChild("Command",$ZipFile =~ /((tgz)|(tar.gz))$/i ? "cd $Target ; tar xvzf $ZipFile" : "cd $Target ; gunzip -d $ZipFile");
	return $Result;
}
...
sub NullCmd
{
	my $Message=shift;
	$Message=shift;
	return  XML::LibXML::Comment->new($Message);
}
...
sub AddInstalltion  ####  ($MdbObj,\%UnitsRec);
{
	...
	my %Operation = ( Copy	=> [\&AddCopy,0] ,
			  ...
			  AddPath => [\&AddPath,2],
			  Unzip	  => [\&unZip,2,\&AddCopy,0],
			  UpdateAgent => [\&NullCmd,"UpdateAgent command should be ignored at SWIM"]);
	my $Err=0;
	my %InstallTree;
	#### Read Instal Table
	...
	foreach my $Row (@CmdList) 
	{
		...
		foreach my $UnitType ( @UnitsList ) 
		{
			...
			foreach my $TmpIdx ( (3..5,8) ) 
			{
				$Row->[$TmpIdx] and exists $G_CLIParams{D} and WrLog("Debug - Parsing Line: \"$Row->[$TmpIdx]\"");
				####  Just Copy String To avoide deletion of data ...
				my $CopyStr=$Row->[$TmpIdx];
				while ( $CopyStr =~ s/\{([^\}]+?)\}//  ) 
				{
					my $ParamName=$1;
					exists $G_ReservedParams{$ParamName} and $Row->[$TmpIdx] =~ s/\{$ParamName\}/$G_ReservedParams{$ParamName}/g and next;
...
sub AddParameter
{
	...
	foreach my $Field (@ParamsList) 
	{
		...
		my $DefVal=  $Field->[1] ? $Field->[1] : $Field->[4];
		my $Descript= $Field->[7] ? $Field->[7] : undef ;
		my $ParamName = $Field->[1] =~ /^\s*\{([^\{\}]+?)\}\s*$/ ? $1 : $Field->[-1] ;
		...
		{     ####    This is Physical Parameter with File Path   ####
			...
			## WrLog("Debug - Parameter  $Field->[-1]  is $ParamName  ($Field->[2])");
		...
		foreach my $UnitType ( @UnitsList ) 
		{
			...
			###### Check for Virtual Params  ####
			foreach my $TmpStr ( @$Field[1,2,4,6] ) 
			{
				my $TmpBuf=$TmpStr ;
				
				## WrLog("Debug - Parameter $Field->[-1] .. ($ParamName) Field val = \"$TmpStr\"");
				
				while ( $TmpBuf =~ s/\{([^\}]+)\}/#InternalParam#/  ) 
				{
					my $ParamName=$1;
					exists $G_ReservedParams{$ParamName} and s/#InternalParam#/$G_ReservedParams{$ParamName}/ and next;
					exists $UnitsRec->{$UnitType}->{VirtualParams} or $UnitsRec->{$UnitType}->{VirtualParams}={};
			...
			if ( @TmpNodes )
			{   ###  The Parameter already exists at CAF File ... ###
				$ParmeterNode=$TmpNodes[0] ;
				my $OldType = $ParmeterNode->findvalue("Restrictions/Type");
				if ( $OldType eq "String" )
				{
					$OldType=$ParmeterNode->getChildrenByTagName("Restrictions")->[0];
					my $TMpMem=$ParmeterNode->replaceChild($TNode->cloneNode(3),$OldType);
				}
				$ParmeterNode->findvalue("Value") and undef $DefVal;
				$ParmeterNode->findvalue("Description") and undef $Descript;
			} else  ###  New Parameter   ###
			{
				$ParmeterNode = $InstallTree{$UnitType}->appendChild($ParmeterNode);
			...
			for ( my $RegExpVal="" ; $DefVal =~ /^(($RegExpVal)\{(.+?)\})/ ; )
			{
				my $PName=$3;
				if ( exists $G_ReservedParams{$PName} ) 
				{
					$DefVal =~ s/^(($RegExpVal)\{.+?\})/$2$G_ReservedParams{$PName}/ ;
					## WrLog("Debug - Reserved parameter found $PName Update Regular Expresion \"$RegExpVal\"");
					$RegExpVal .= $G_ReservedParams{$PName} ;
				} else
				{
					$PName eq $ParamName and ( undef($DefVal) , last ) ;
					$RegExpVal = $1;
				}
				## WrLog("Debug - RegExp = $RegExpVal ... ($DefVal)");
			}
			...
			if ( $ConstValFlag ) 
			{
				...
				{
					$ParmeterNode->setAttribute("Display","false");
				}
			}
...
my $ErrFlag;
my %GroupMap;
#### Map Groups to OctopusUnits
while ( my ($OctUName,$GrpRec) = each (%UnitMap) )
{
	exists $GroupMap{$GrpRec->{GroupName}} or $GroupMap{$GrpRec->{GroupName}}=[];
	push(@{$GroupMap{$GrpRec->{GroupName}}},$OctUName);
}
...
foreach my $MdbFileName ( getFileList(split(',',$G_CLIParams{File})) ) 
{
	...
	unless ( $G_ErrorCounter > $ErrFlag and ! exists $G_CLIParams{IgnoreError} ) 
	{
		...
		{
			exists $G_CLIParams{D} and WrLog("Debug - Going to save $UnitType CAF File");
			...
			#### Merge UAF which map to same Group ...
			exists $RecPoint->{UAFXml} or next;
			my $TargetUAF=$RecPoint->{UAFXml};
			$TargetUAF= $TargetUAF->findnodes("//Install")->[0];
			foreach my $OctUnit ( @{$GroupMap{$RecPoint->{GroupName}}} )
			{
				$OctUnit eq $UnitType and next;
				exists $UListRec{$OctUnit} or next;
				exists $UListRec{$OctUnit}->{UAFXml} or next;
				my $RefXml=$UListRec{$OctUnit}->{UAFXml};
				exists $G_CLIParams{D} and WrLog("Debug - Merge UAF of $UnitType with $OctUnit");
				foreach my $CompNode ( $UListRec{$OctUnit}->{UAFXml}->findnodes("//Component") )
				{
					my $TmpCompName=$CompNode->getAttribute("Name");
					$TargetUAF->findnodes("//Component[\@Name=\"$TmpCompName\"]") and next;
					$TargetUAF->appendChild($CompNode);
				}
				delete $UListRec{$OctUnit}->{UAFXml};
				delete $UListRec{$OctUnit}->{UAFFileName};
			}
		    ...
		    WrLog("Info  - UAFFile $RecPoint->{UAFFileName} have been UpDated");
		}
	}
