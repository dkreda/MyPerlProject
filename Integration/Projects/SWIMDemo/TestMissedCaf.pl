...
sub usage
{
    print "$0 -Kits Kits folder [-Sys System folder][-LogFile FileName]\n";
    print "$0 -help\n";
}
...
WrLog(@DebugMessages);
my (%CompList,%MissedCAFs);
### Read All UAF File  ###
my @UAFFiles=SearchFiles($G_CLIParams{Sys},"UAF.xml\$");
foreach my $UAFFileName (@UAFFiles)
{
	WrLog("Info - Parrsing $UAFFileName");
	my $XmlObj=XML::LibXML->new()->parse_file($UAFFileName);
	foreach my $CompObj ( $XmlObj->findnodes("//Component") )
	{
		my $CompName=$CompObj->getAttribute("Name");
		$CompList{$CompName}="NotExists";
	}
}

####   Search All CAFs at Kits Folder  ###
my @CAFFiles=SearchFiles($G_CLIParams{Kits},"CAF.xml\$");

foreach my $CAFFileName (@CAFFiles)
{
	## WrLog("Debug - Parrsing $CAFFileName");
	my $XmlObj=XML::LibXML->new()->parse_file($CAFFileName);
	my $CompName=$XmlObj->findvalue(q(//Component/@Name));
	exists $CompList{$CompName} and $CompList{$CompName}="Find" , next;
	$MissedCAFs{$CompName}=$CAFFileName;
}
@DebugMessages=@CAFFiles > 0 ? (sprintf("%-35s | %-60s","Component Name","CAF File Name"),'=' x 91) : ("Error - No CAF File Found at ","\t\"$G_CLIParams{Kits}\"",
			"\tverify you entered Valid Path at -Kits Parameter");
while ( my ($CName,$FName) = each (%MissedCAFs) )
{
	push(@DebugMessages,sprintf("%-35s | %-60s",$CName,$FName));
}
if ( @DebugMessages > 2 )
{
	WrLog("List of missed CAFs:",@DebugMessages);
	$G_ErrorCounter++;
} else
{
	WrLog("All The CAFs at $G_CLIParams{Kits} are used by the system");
}
