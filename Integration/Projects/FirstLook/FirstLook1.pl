   ...
   do {
      ...
      if ($mhn eq "LOCAL") {
         ...
         chop $hostname;
            ...
            next if (($hostname = &loginpass($mhn)) eq "");
            }
      $cidx = 0;
      $scriptcmd = "";
       ...
       if ($errout =~ /Permission denied/) {
          ...
          print "\nRetrying connection...\n";
           ...
           if ($outout < 31) {
              ...
              print "\rCPU idle on $host is only $outout% . FL commands on this host will be skipped.\n";
              &writeout("CPU idle on $host is only $outout% . FL commands on this host will be skipped.");
              return("");
              }
           $ok = 1; 
           }
...
sub writeout {
    ...
    print wo "$output\n";
    close(wo);
    $endstatus = $args{errorlevel} if (($? != 0) && ($endstatus < $args{errorlevel}));
    open (STDERR,">/dev/tty");
    }
...
sub wrapup {
    ...
    if ($endstatus == 1) {
       ...
       &writeout("****NOTE Some of the commands executed failed. Could be indicative of researched problem");
          ...
          print "Please contact Comverse support for assitance.\n";
         ...
         print "\nDone.\n";
         &writeout("FirstLook collection completed successfuly.");
         }
...
$run_func = $ARGV[0] || die "usage: collect.pl func [area]\n";
$run_common = $ARGV[1];
...
print "\nCollection may take up to 10-15 minuets,\n";
print "Depends on the system and the info collected\n";

`echo Starting information collection > $OUTFILE`;
`date >> $OUTFILE`;

open (cfg,"cat def/collect.def def/collect.$run_common.common.def def/collect.$run_func.def |");
