...
my $SshObj=Net::SSH2->new(trace => -1 );
WrLog("Connecting to 10.106.50.185 ...");
$SshObj->connect("10.106.50.185");
WrLog("Authenticating ....");
#$SshObj->auth_password("root",'Adm1Cmv4$');
$SshObj->auth_password("admin",'aDm4cu2t');
WrLog("Open Chaneel ...");
my $SSHChanel=$SshObj->channel();

$SSHChanel->shell();
print $SSHChanel "wall \"Hellow this is admin\"\n";
print "Line : $_\n" while <$SSHChanel> ;
print $SSHChanel "whoami\n" ;
print "Line : $_\n" while <$SSHChanel> ;
print $SSHChanel "su -\n" ;
print "Line : $_\n" while <$SSHChanel> ;
print $SSHChanel "aDm4cu2t\n";
print $SSHChanel "whoami\n";
$SSHChanel->close;

#$SSHChanel->blocking(1);
my @OutList=$SSHChanel->exec("wall \"Hellow World\"");
$SSHChanel->send_eof;

#$SshObj->login("root", 'Adm1Cmv4\$');
#my @OutList=$SshObj->cmd("pwd ; ls -l");
WrLog("DD - @OutList",@OutList,"DDD :",$SSHChanel->exit_status);
