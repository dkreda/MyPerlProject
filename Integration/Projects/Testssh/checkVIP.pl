...
{
    my $ChPID=shift;
    my @PIDList=`ps -f -p $ChPID`;
    @PIDList > 1 or return  undef;
    $PIDList[-1] =~ /((\S+)\s*){3}/ or return  undef;
    return $2;
}
...
{
    my $SPID=shift;
    my @Resource=`lsof -np $SPID`;
    chomp(@Resource);
    my @Result;
    foreach my $Iter (grep(/IPv4.+ESTABLISHED/,@Resource))
    {
    	$Iter =~/TCP\s+(\S+?)->/ or next;
    	push(@Result,$1);
    }
    return @Result;
}
...
WrLog(@DebugMessages);
my $MatchID=$$;
WrLog(" - Current Process ID: $MatchID");
my @Stam=`ps -ej`;

chomp (@Stam);

my @Relevant=grep(/$MatchID\s+((\d+)\s+){2}/,@Stam);
#my @SearchList;
$Relevant[0] =~ /((\d+)\s+){3}/ and $MatchID=($2);
WrLog(" - Match Process:",$Stam[0],@Relevant,$2);

while ($MatchID)
{
    WrLog("-Search relevant sockets for process: $MatchID");
    @Relevant=findSoket($MatchID);
    $MatchID = @Relevant ? undef : getParent($MatchID);
}
WrLog(" - is this the relevant connection:",@Relevant);
my @CheckList;
my @HostList=ReadFile("/etc/hosts");
foreach my $Host ( split(',',$G_CLIParams{Host}) )
{
    unless ( $Host =~ /(\d+\.){3}\d+/ )
    {
    	foreach my $Iter (grep(/$Host/i,@HostList))
    	{
    	    $Iter =~ /^\s*([\d\.]+)/ or next;
    	    $Host=$1;
    	    last;
...
foreach my $Iter (@Relevant)
{
     $Iter =~ s/:\S+// ;
     grep(/^$Iter$/,@CheckList) or next ;
     $G_ErrorCounter++;
     WrLog("Error - This scripts is running above $Iter session ...");
     last;
}
