...
my %actions = (
  ...
  usingssh => sub {
	  exists $G_CLIArgs{Password} and $passwords{"default:$args{usingssh}"}=$G_CLIArgs{Password}->[0];
	  my $TTT="default:$args{usingssh}";
      unless ($run_common eq $RUNS_ON or exists $passwords{"default:$args{usingssh}"} ) {
             print "\n**\n**\n**\n";
  ...
  resetvar => sub {
	my $resarg=$args{resetvar};
	exists $xdict{$resarg} and delete $xdict{$resarg};
	} ,
  reset => sub {
     my $resarg;
...
sub precheck {
	...
	sub ReadCLI
	{
		...
		unless ( exists $G_CLIArgs{Files} )
		{
			my @PList=("collect.$ARGV[2].main.def","collect.$ARGV[1].common.def","collect.$ARGV[0].def");
			$G_CLIArgs{Files}=\@PList;
		}
		foreach my $DefFile (@{$G_CLIArgs{Files}})
		{
			my $FLocation = -f $DefFile ? $DefFile : "$FLDIR/def/$DefFile" ;
			-f $FLocation or die "Fatal - definision File $DefFile Not exists\n";
			open SIN , "<$FLocation";
			my @Lines=<SIN>;
			close SIN;
			chomp (@Lines);
		...
		$run_acc or die "usage: collect.pl func area acc\n";
		$run_common or die "usage: collect.pl func area acc\n";
		$run_func or die "usage: collect.pl func area acc\n";
	}
...
my $FList;
foreach my $FName ( @{$G_CLIArgs{Files}} )
{
	$FList .= -f $FName ? " $FName" : " $FLDIR/def/$FName" ;
}
open (cfg,"cat -s$FList |");
