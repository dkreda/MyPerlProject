...
###############################################################################
{
   ...
   if ( $Err )
   {
   	...
   	$G_ErrorCounter++;
	###  just reset error level  ###
	while ( $? ) { system("echo > /dev/nul"); }
   	return ;
   }
   chomp (@Result);
   return @Result;
...
sub RunSSH # $Host , $user ,$password ,@Cmds
{
use Net::SSH::Perl ;
	my $Host=shift;
	my $User=shift;
	...
	my $SSHObj=Net::SSH::Perl->new($Host,%SSHOp);
	WrLog("New SSh Connection started ..");
	my $ErrRes=$SSHObj->login($User,$Password);
	WrLog("- Login successful ? ($ErrRes)");
	($StdOut,$StdErr,$Err)=$SSHObj->cmd($Cmds[0]);
	WrLog("-SSh Resullt (exit code) - $Err",
		  "- StdOut: $StdOut",
		  "--------------------------------------------",
		  "- StdErr: $StdErr");
	return;
...
my $ErrMessage= $G_ErrorCounter ? "Finish with ERRORS see Log File $G_CLIParams{LogFile}" :
		"Finish Successfully :-)";
RunSSH($G_CLIParams{Host},"root",'Adm1Cmv4$',"whoami ; ls -ltr");
