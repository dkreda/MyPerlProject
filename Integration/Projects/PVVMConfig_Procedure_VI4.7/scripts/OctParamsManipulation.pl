...
###############################################################################
{
   ...
   my %OctVals=ReadSections($ConfigPointer,"Octopus.Parameters.Values");
...
###############################################################################
{
    ...
    my $Err=$G_ErrorCounter;
    while ( my ($PVal,$PCal) = each(%MultVal) )
    {
    	$ActValue =~ /(-?\d+(\.\d+)?)/ or $G_ErrorCounter++;
    	$G_ErrorCounter > $Err and WrLog("Error - Ilegle Parameter value ($ActValue) for Multiple operation"),return ;
    	foreach my $Val (split(',',$PCal) )
    	{
    	   unless ( $Val =~ /(-?\d+(\.\d+)?)/ )
    	   {
    	   	$G_ErrorCounter++ ;
    	   	WrLog("Error - Ilegle Value assignment \"$Val\" for Multiple mapping Rule. should be Number.");
    	   	last;
    	   }
    	   push(@Result,$1*$ActValue);
    	   defined $G_CLIParams{D} and WrLog("Debug - Update $Result[-1]");
    	}
    	$G_ErrorCounter > $Err or return @Result;
    }
    while ( my ($PVal,$PCal) = each(%AddVal) )
    {
    	$ActValue =~ /(-?\d+(\.\d+)?)/ or $G_ErrorCounter++;
    	$G_ErrorCounter > $Err and WrLog("Error - Ilegle Parameter value ($ActValue) for Plus operation"),return ;
    	foreach my $Val (split(',',$PCal) )
    	{
    	   unless ( $Val =~ /(-?\d+(\.\d+)?)/ )
    	   {
...
###############################################################################
{
    ...
    my %ParamsValue=ReadSections($ConfPointer,"Octopus.Parameters.Values");
    ...
    $Err == $G_ErrorCounter or WrLog("Error - missing configuration section \"RealName.Parameters.Def\"",
    				     "        or \"Octopus.Parameters.Values\" at $$ConfPointer{FileName}"),return 1;
...
$G_ErrorCounter += SetSection(\%Configuration,"Octopus.Parameters.Values",%CLIOctParams);
my %TmpOctopusParams=ReadSections(\%Configuration,"Octopus.Parameters.Values");
...
unless ( $G_ErrorCounter )
{
    ...
    $G_ErrorCounter += SetSection(\%Configuration,"Octopus.Parameters.Values",%RealConfList);
    %TmpOctopusParams=ReadSections(\%Configuration,"Octopus.Parameters.Values");
